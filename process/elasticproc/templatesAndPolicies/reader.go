...
import (
	...
	"github.com/TerraDharitri/drt-go-chain-es-indexer/templates/indices"
)
...
func (tr *templatesAndPolicyReader) GetElasticTemplatesAndPolicies() (map[string]*bytes.Buffer, map[string]*bytes.Buffer, error) {
	...
	indexTemplates[indexer.AccountsDCDTIndex] = indices.AccountsDCDT.ToBuffer()
	indexTemplates[indexer.AccountsDCDTHistoryIndex] = indices.AccountsDCDTHistory.ToBuffer()
	...
	indexTemplates[indexer.DCDTsIndex] = indices.DCDTs.ToBuffer()
...
func (tr *templatesAndPolicyReader) GetTimestampMsMappings() ([]templates.ExtraMapping, error) {
	return []templates.ExtraMapping{
		...
		{
			Index:    indexer.AccountsDCDTIndex,
			Mappings: indices.TimestampMs.ToBuffer(),
		},
		{
		...
		{
			Index:    indexer.AccountsDCDTHistoryIndex,
			Mappings: indices.TimestampMs.ToBuffer(),
		},
		{
